...
Tutto il sistema si adatta al livello. Le feature si sbloccano progressivamente.
Per lo sviluppo iniziale, costruire TUTTE le feature sbloccate. Il sistema di lock/unlock per livello verrà aggiunto alla fine come ultimo step.
...
Vede tutto sbloccato sempre. Accesso a pannello admin.
Determinato da campo `role` in profiles.
...
"Hai già un annuncio su Airbnb?"
→ "Sì" (mostra campo URL) / "No, sto iniziando"
...
⛔ Non toccare la logica di analisi.
**Aggiungere:**
...
Badge:
- 🏠 Prima Analisi
- ✏️ Wordsmith (generato e applicato titolo)
...
- 📊 Analista (3+ analisi)
- 🗺️ Guida Perfetta (creato check-in + local guide)
- 💰 Revenue Master (stima revenue calcolata)
UI: grid badge, locked/unlocked, confetti su unlock

### F28. Notifications (/notifications)
Lista tutte le notifiche con read/unread.
Bell icon nella dashboard con badge count.
...
**Features SENZA AI (puro calcolo/dati):**
Pricing Coach, Calendar, Revenue Estimator, Amenity Gap, Superhost Tracker, Accessibility Checker, Regulation Helper, Tax Helper, Achievement System, Notifications, Alerts, First Impression Simulator (partial), Multi-listing, Admin Panel
